...
class SeatChooser extends React.Component {
  ...
  componentDidMount() {
    ...
    loadSeats();
    this.socket = io();
    this.socket.connect((process.env.NODE_ENV === 'production') ? '/' : 'http://localhost:8000/');
  ...
  render() {
    ...
    return (
      <div>
        ...
        <h4 className="text-muted ml-2">Free seats: { 50 - seats.filter(seat => seat.day == chosenDay).length }/50</h4>
      </div>
    )
  };
}
